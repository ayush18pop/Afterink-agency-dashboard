...
export function useTaskTracker({ onStatusChange }) {
  ...
  const startTask = async (taskId) => {
    try {
      await axios.post('/api/time/start', { taskId }, { withCredentials: true });
  ...
  const holdTask = async () => {
    if (activeTaskId) {
      try {
        await axios.post('/api/time/hold', { taskId: activeTaskId }, { withCredentials: true });
        setStatus('Hold');
        stopTimer();
  ...
  const completeTask = async () => {
    if (activeTaskId) {
      try {
        await axios.post('/api/time/complete', { taskId: activeTaskId }, { withCredentials: true });
        setStatus('Completed');
        stopTimer();
